...
复用边界（plan D9；2026-09-17 迁出独立后修订）：
- `assert_readonly_select` import 自 `app.sqlguard`（data_source 守卫的本地单一真源拷贝——
  独立服务与 gateway 跨 venv 无法再共享单一真源；拷贝来源与同步注意见 app/sqlguard.py 头注）
...
from app.ontology.schemas import AccessConfig
from app.sqlguard import _build_db_url, assert_readonly_select
