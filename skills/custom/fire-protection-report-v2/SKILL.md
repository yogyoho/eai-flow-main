...
description: |
  ⛔ 如果用户已上传设计说明书(.docx)，不要使用本技能，改用 fire-protection-extract 技能。
  本技能仅在用户没有上传设计说明书时使用。
  ...
  ⚠ 以上触发词仅当用户没有上传设计说明书(.docx)时适用。有设计说明书时路由到 fire-protection-extract。
---
...
以下是生成各章内容时的核心要点，详细说明见 `references/content_guidelines.md`。

### 第1章：设计依据及采用的标准
- 列出项目批准文件（可研报告批复、部门来函等）
- 列举所有适用的 GB 标准（注意版本年份）
- 包含国家和地方消防法规
...
### 第4章：防火安全措施
- 总平面布置和防火间距
- 防雷设计原则（防雷等级、接闪器、引下线、接地体）和接地设计
- 供电安全（负荷等级、电源、配电、应急电源）
- 建筑防火（建构筑物一览表：面积、高度、火灾危险性、耐火等级、结构类型）
- 建筑物通风措施（通风特性表）
