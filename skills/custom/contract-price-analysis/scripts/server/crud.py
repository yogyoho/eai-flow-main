...
Each function takes an ``AsyncSession`` and returns ORM objects or primitives.
Query construction is separated from the routers so it is unit-testable with a
mocked session. All functions assume the schema exists (``init_schema`` ran).
"""
...
from scripts.server.schemas import ConfigOut
...
async def delete_document(session: AsyncSession, doc_id: UUID) -> bool:
    # Cascade-delete items whose document_id matches, then the document.
...
async def confirm_cluster(
    ...
    if expected_version is not None and cluster.version != expected_version:
        raise ValueError(
            f"version mismatch: expected {expected_version}, got {cluster.version}"
        )
...
async def merge_clusters(
    ...
    # Re-point all items to a new cluster, then delete the old clusters.
    ...
    await session.execute(
        update(CpaItem).where(CpaItem.cluster_id.in_(cluster_ids)).values(cluster_id=new_cluster.id)
    )
...
async def move_item(session: AsyncSession, item_id: UUID, target_cluster_id: UUID) -> Optional[CpaItem]:
    ...
    await session.commit()
    # Recount both clusters' item_count lazily is fine; leave counts to recompute.
    return item
...
async def dashboard_counts(session: AsyncSession) -> dict:
    ...
    pending = await session.scalar(
        select(func.count()).select_from(select(CpaCluster).where(CpaCluster.status == "pending").subquery())
    ) or 0
    confirmed = await session.scalar(
        select(func.count()).select_from(select(CpaCluster).where(CpaCluster.status == "confirmed").subquery())
    ) or 0
    return {
...
async def price_range(session: AsyncSession) -> Optional[dict]:
    row = await session.execute(
        select(func.min(CpaItem.unit_price), func.max(CpaItem.unit_price), func.avg(CpaItem.unit_price))
    )
    lo, hi, avg = row.one()
    if lo is None:
        return None
