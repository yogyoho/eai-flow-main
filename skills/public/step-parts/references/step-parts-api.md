...
> **EAI 部署注意(先读)**:本文档是上游逐字参考,**agent 无法直连这些端点** ——
> 在本部署中 MCP 工具 `text-to-cad_search_step_parts` 是访问 step.parts 的**唯一通道**
> (参数仅 `query`/`limit`/`standard`,下载模式 `download_id`+`output_path`;`standard`
> 对应下文的 `standard` facet;其余 facet/翻页参数未暴露)。以下内容用于理解查询语义、
> 记录字段与下载/校验行为,不要照此文档发起 HTTP 请求。
