# EAI-CUSTOM: forked from contract-price-analysis/scripts/models.py。
"""SQLAlchemy ORM for csp_ tables(备件价格体系分析)。

镜像 backend/app/extensions/spare_parts/models.py —— 两者共享同一套物理表(各自
独立的 Base),改字段必须两边同步。

与 contract_price (cpa_) 的差异(D3):
  - 新增 CspCustomer 客户主数据表(canonical_name + aliases 别名映射)。
  - CspDocument 加 customer_id / customer_name(采购方/需方 = 分析维度)。
  - CspItem 加 customer_id / customer_name(冗余,便于按客户聚合比价);
    goods_name → part_name,spec_model → spec。
  - CspRunHistory 加 customers_resolved(归一命中的客户数),去 excel_path(MVP defer)。
"""
...
class CspCustomer(Base):
    ...
    未命中 → status=pending 入待确认队列,人工认领/合并。
    """
    ...
    aliases: Mapped[Optional[list]] = mapped_column(JSONB)  # ["别名1","别名2",...]
    source: Mapped[Optional[str]] = mapped_column(String(100))  # master/imported/ocr
...
class CspDocument(Base):
    """备件合同扫描件。bucket=csp-parts。customer_id 为采购方(需方)。"""
    ...
    quick_fp: Mapped[Optional[str]] = mapped_column(String(256))  # name|size 快速预过滤
    contract_no: Mapped[Optional[str]] = mapped_column(String(100))
    supplier: Mapped[Optional[str]] = mapped_column(String(200))  # 供方/卖方(文档元数据)
    customer_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), ForeignKey("csp_customers.id"), index=True
    )  # 需方/买方 = 分析维度(D3)
    customer_name: Mapped[Optional[str]] = mapped_column(String(200))  # 冗余,OCR 原文
    project_name: Mapped[Optional[str]] = mapped_column(String(300))
    project_location: Mapped[Optional[str]] = mapped_column(String(300))
...
class CspItem(Base):
    ...
    spec: Mapped[Optional[str]] = mapped_column(String(300))  # 规格/型号
    ...
    customer_name: Mapped[Optional[str]] = mapped_column(String(200))  # 冗余
    ...
    # ── traceability (OCR bbox byproduct) ──
