...

{
  ...
  "path": "<章节标题链,如 投标文件格式/三、法定代表人身份证明（格式）>",
  "slot_type": "text | table | image | format_check | group",
  "required_format": {"desc": "<格式要求/待填内容提示|null>", "table_spec": {"columns": ["<列头1>", "<列头2>"], "rows": <数据行数,整数≥1>|null, "fixed_rows": [["<固定行单元格逐字照抄>"]]|null}, "template_text": "<模板固定文字原文逐字照抄|null,≤10000字>", 或 null},
  "linked_clause_ids": ["<关联条款 clause_id,无则空数组>"]
}

硬性纪律:
- **path 逐字照抄(1:1 复刻第一道关)**:章节结构只镜像招标文件规定,绝不自创章节/改写标题;path 的每一段必须是原文标题的**逐字拷贝**——
  保留数字编号、（格式）类后缀、全角/半角、标点与空格,禁止任何归一化(去编号/去后缀/半角化/并空格/同义改写/繁简转换)。
  反例(回放实证):"1. 投标书（格式）"写成"投标书"——编号与后缀被剥,即不合格。照抄是否合格由 check_format.py 对源文件逐字校验,
  对不上=异常项进确认门1,不存在"差不多就行"。
- **骨架零丢弃**:格式章节骨架里的每个标题、每张表都必须产出对应节点,禁止静默丢项(回放实证:封面/资格声明/授权书整个节消失)。
  check_format.py 会对源文件标题做骨架覆盖点名,漏一个就报一个。
- slot_type 判定:文字内容=text;要求复刻表格=table(table_spec 形状钉死:columns=非空列头字符串数组,与源表格列头逐字一致;
  rows=数据行数整数≥1(缺省按 1)——不要把行数据数组放进 rows,空白待填行渲染时人工填;**fixed_rows=表格模板既有固定行**
  (示例行/说明行等非空白行)的逐字拷贝,二维数组、每行列数=columns 长度、待填单元格写空串,无法辨认时 null 不猜);
  需插入证件/图纸扫描件=image;签字/盖章/份数/页码/目录等非内容格式义务=format_check;纯容器=group。
- template_text 模板原文预填:招标文件中所有商务类格式模板——包括且不限于投标响应函、法定代表人授权委托书、
  报价一览表、分项价格表、投标货物分项报价明细表、商务偏离表(项目要求及报价响应表)、技术偏离表(技术条款响应/偏离表)等——
  的固定文字(函件正文/声明文/承诺文,表格模板的列头与固定行文字,格式章节的固定说明散文)必须逐字照抄进 template_text,
  不得改写/概括/拼接/换行重排;chunk 原文不存在该模板文字时写 null,绝不编造。照抄由 check_format.py 逐字校验兜底,终稿再由人核清单逐字比对。
