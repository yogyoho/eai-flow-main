...
1. **开题首动作三件套（bug-2231 页面实测：开题第一轮一轮做完，不可拆分、不可只说不做）**——按序完成：① **真实调用** KF MCP `kf_resolve_template`（工具全名 `knowledge-factory_kf_resolve_template`，报告模板+标准，与用户是否已给阶段无关），必须在本回复留下实际工具调用记录——口头声称"已解析 / found=false"而未调用 = 未做（工具不可用或调用失败视同 `found=false`，进 ②；**`found=false` 且 `reason=missing_keywords` 时须按工具 suggestion 补 `domain_keywords`（如 `["固体矿产","地质勘查",<矿种>]`）重试 ≤1 次，仍 false 才兜底**——bug-3066 页面实测：该工具硬性要求 domain_keywords，缺参时连模板库都不查即返回 false）；② `found=false` 时向用户声明：
   ...
   **矿种知识包装载（P5 数据驱动）**：commodity 归一化确定后（用户首消息标题/描述或 00_project 表单，如 岩金→gold），**首读** `references/ore_packs/<矿种>.json` 矿种知识包（基本分析项目/物相分带/伴生组分政策/小体重实践/典型矿床式/报告要点，均机器可校验）——后续领域判断（工业指标、基本分析项目、伴生组分、规范引用）以数据包为准；**无该文件 = 该矿种尚未孵化**，回退下方「领域速记」prose 硬编码知识，并向用户声明一句「该矿种暂无知识包，按规范通用规则+你提供的工业指标执行」，绝不因缺包编造矿种知识。
...
**矿种适配（数据包优先，P5）**：优先读 `references/ore_packs/<矿种>.json` 矿种知识包（机器可校验；包内【待核实】标记项仍须对照 standards_index 人工核实后才可写成断言）；无包回退 prose 兜底——铜矿基本分析 Cu/Ag/Au/S/As，伴生组分单独圈定估算（formulas L11 估算链）；煤矿用灰分/发热量/硫分指标且 DZ/T 0215-2020 无泥炭内容。其他矿种引导用户提供工业指标。
