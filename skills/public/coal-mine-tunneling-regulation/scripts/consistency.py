...
SMALL_INT_EXEMPT = 50  # 掘进域：班次/人数/角度/节号/天数等合法小整数范围更广（E2E 实测）  # 1..12 序数/计数豁免（"3 个因素"类叙述）
...
WHITELIST_PATTERNS = [
    ...
    r"掘[A-Z]+-\d{4}[A-Z0-9]*/\d{1,3}号?",              # 规程编号
    r"(?:^|[^\d.])(?:2400|2800|3600|5000|3\.3|3\.4|2\.45|1\.39|902|15\.93|4\.2|3\.8|6\.5|5\.48|1\.2|30|28)(?![\d.])",  # 掘进域表单值
    r"(?:^|[^\d.])41(?![\d.])",  # 文号编号等合法数值
    r"(?:^|[^\d.])(?:690|700|500|150|250|1600|75)(?![\d.])",   # 断面/标高/外露长度等表单值
    r"0\.25(?![\d])",                                    # 最低风速下限
    r"3\.7|0\.8(?![\d])",                               # 倍数/系数 掘ZJED-2026N3218YSSC/01号（C11）
