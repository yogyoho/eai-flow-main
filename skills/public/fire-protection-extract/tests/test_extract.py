...
def _tiny_mapping():
    return {
        "sources": [
            [{"kind": "para", "paras": [7]}],
            [{"kind": "para", "paras": [12], "authoritative": True}],
            [{"kind": "table", "no": "表2.1-1"}],
            None,
        ]
    }
...
def test_extract_verbatim_para_and_table(tiny_spec):
    body, cites = extract(_structure(tiny_spec), _tiny_outline(), _tiny_mapping())
...
def test_extract_conflict_uses_authoritative_not_wrong_source(tiny_spec):
    body, _ = extract(_structure(tiny_spec), _tiny_outline(), _tiny_mapping())
...
def test_missing_anchor_is_flagged_not_silent(tiny_spec):
    m = _tiny_mapping()
    m["sources"][0] = [{"kind": "para", "paras": [9999]}]
    body, _ = extract(_structure(tiny_spec), _tiny_outline(), m)
    assert "[⚠未找到段落" in body


def test_build_report_has_title_and_headings(tiny_spec):
    md = build_report(_structure(tiny_spec), _tiny_outline(), _tiny_mapping(), project_name="基地项目")
