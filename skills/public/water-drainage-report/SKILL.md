...
- **table 章**（参数表/工艺计算表/设备表）= 纯公式输出 + `traces.json` 机械渲染，**不走 LLM**：最快、最准、天然带步骤轨迹。计算过程块必须脚本注入不得手写（历史：R4/R5/R6 三轮实测 agent 手写从不产 `<details>` 折叠；R6 即便生成了片段也不逐字粘贴——6K 字符复制对 LLM 不可靠；**R8 实测 agent 把 12 块全文手写进 write_file 并跳过 inject**，故 2026-08-29 起由快照门禁强制）：
  ...
  ⛔ 禁止手写 KaTeX 公式块/计算过程折叠块替代脚本注入——**全文任何位置**（含 narrative 章的水池/泵房/旁滤小节）都不许写 `<details>` 或 `$$` 公式块；narrative 章引用数值用纯文本并标注"计算见第X章"。**快照门禁（R8+R9）**：`snapshot.py save --report ...` 校验报告——① 含未注入占位符；② 含无签名手写 `<details>`；③ `<details>` 总数 ≠ 注入签名块数（R9 实测：注入 12 块后又在第6-8章手写 8 块、其中 V_ratio 单位抄错成"0.202 h"）→ 一律 `SNAPSHOT_ERROR` 退出 1。打回后必须删除全部手写折叠块（保留唯一占位符注入产物）再 save。**④（R11，正向）**：报告须含注入签名 `CALC_BLOCKS_INJECTED:v2` 且不得残留未注入 `<!-- CALC:id -->` 占位符——整份手写报告（0 details + 0 占位符 + 0 签名，b2117e88 实测穿透①②③）同样 `SNAPSHOT_ERROR` 打回，**跳过 inject 没有出路**。**⑤（R12，插图引用）**：每个图片行必须独占一行、文件名必须是契约形态 `images/<12位hex>.<扩展名>` 且真实存在于 `outputs/images/`；盘上有图而报告零引用、或盘上无图且无【插图待人工补充】占位（插图步骤整跳）一律打回——手造文件名（bug-3017/bug-3039 两次复发，b7c56031 三个全错）在 save 即拦，文件名只准逐字复制 `DIAGRAM_FILE:` 输出。
