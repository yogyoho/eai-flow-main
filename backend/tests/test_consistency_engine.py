...
def _contracts_path() -> Path:
    return Path(__file__).resolve().parent.parent.parent / "skills" / "custom" / "water-drainage-report" / "references" / "consistency_contracts.json"


def _build_water_graph(Q: float = 20000, N: float = 5, V_suction: float = 2099.5) -> FormulaGraph:
    """Build the standard 12-formula water system graph."""
    graph = FormulaGraph()
    nodes = [
        FormulaNode("Qe", "蒸发水量", "6.1.1", "Q * KZF * delta_t", inputs={"Q": ParamSource.user(Q), "KZF": ParamSource.lookup(0.001461), "delta_t": ParamSource.user(10)}, outputs={"Qe": "m3/h"}),
        FormulaNode("Qw", "风吹损失水量", "6.1.2", "Q * 0.001", inputs={"Q": ParamSource.user(Q)}, outputs={"Qw": "m3/h"}),
        FormulaNode("Qb", "排污水量", "6.1.3", "Qe / (N - 1)", inputs={"Qe": ParamSource.from_formula("Qe", "Qe"), "N": ParamSource.user(N)}, outputs={"Qb": "m3/h"}),
        FormulaNode("Qm", "补充水量", "6.1.4", "Qe + Qw + Qb", inputs={"Qe": ParamSource.from_formula("Qe", "Qe"), "Qw": ParamSource.from_formula("Qw", "Qw"), "Qb": ParamSource.from_formula("Qb", "Qb")}, outputs={"Qm": "m3/h"}),
        FormulaNode("V_pool", "塔底水池有效容积", "7.1.1", "pool_area * effective_depth", inputs={"pool_area": ParamSource.user(912), "effective_depth": ParamSource.code(2.0)}, outputs={"V_pool": "m3"}),
        FormulaNode("V_system", "系统总容积", "7.1.1", "V_pool + V_suction", inputs={"V_pool": ParamSource.from_formula("V_pool", "V_pool"), "V_suction": ParamSource.user(V_suction)}, outputs={"V_system": "m3"}),
        FormulaNode("V_ratio_check", "系统容积比校验", "7.1.1", "V_system / Q", inputs={"V_system": ParamSource.from_formula("V_system", "V_system"), "Q": ParamSource.user(Q)}, outputs={"V_ratio_check": ""}),
        FormulaNode("pump_foundation_L", "水泵基础长度", "8.2.1", "pump_motor_spacing + 0.5", inputs={"pump_motor_spacing": ParamSource.user(5.2)}, outputs={"pump_foundation_L": "m"}),
        FormulaNode("Qsf", "旁滤处理水量", "9.1.1", "Q * sf_ratio", inputs={"Q": ParamSource.user(Q), "sf_ratio": ParamSource.code(0.05)}, outputs={"Qsf": "m3/h"}),
        FormulaNode("filter_count", "过滤器台数", "9.1.2", "math.ceil(Qsf / filter_unit_capacity)", inputs={"Qsf": ParamSource.from_formula("Qsf", "Qsf"), "filter_unit_capacity": ParamSource.user(40)}, outputs={"filter_count": "台"}),
        FormulaNode(
            "backwash_flow",
            "反洗瞬时流量",
            "9.1.3",
            "filter_area * backwash_intensity * concurrent_backwash",
            inputs={"filter_area": ParamSource.user(1.13), "backwash_intensity": ParamSource.user(15.0), "concurrent_backwash": ParamSource.user(5)},
        ...
        FormulaNode(
            ...
            inputs={"backwash_flow": ParamSource.from_formula("backwash_flow", "backwash_flow"), "backwash_duration": ParamSource.user(2.0), "total_filters": ParamSource.from_formula("filter_count", "filter_count")},
...
class TestCrossDiscipline:
    def test_matching_values_pass(self):
        ...
        engine.add_contract(
            Contract(
                id="fire-water",
                type=ContractType.CROSS_DISCIPLINE,
                param_name="消防水量",
                rule="exact_match",
                severity=Severity.FAIL,
                sources=[
...
class TestWaterDrainageScenario:
    ...
    def test_fire_water_cross_discipline(self):
        ...
        report = engine.check_report()
        fire_violations = [v for v in report["violations"] if v["contract_id"] == "fire-water-cross-discipline"]
    ...
    def test_report_structure(self):
        ...
        engine.add_contract(
            Contract(
                ...
                type=ContractType.CROSS_SECTION,
                param_name="x",
                sources=[{"section": "1", "value": 1}, {"section": "2", "value": 1}],
            )
        )
        engine.add_contract(
            Contract(
