...
class TestDynamicContextMemoryStamping:
    ...
    def test_the_memory_block_is_stamped_as_memory(self, monkeypatch):
        ...
        # EAI-CUSTOM (bug-697): _build_full_reminder also receives thread_id for
        # project-context reads — the stub must accept it. Upstream additionally
        # passes a keyword-only ``query``; the stub accepts both.
        monkeypatch.setattr(
            module.DynamicContextMiddleware,
            "_build_full_reminder",
            lambda self, runtime=None, thread_id=None, *, query=None: ("<system-reminder></system-reminder>", "some recalled memory"),
        )
