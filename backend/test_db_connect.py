...
async def test(port, db, user, pw):
    try:
        conn = await asyncpg.connect(
            host='localhost', port=port, user=user,
            password=pw, database=db, timeout=5
        )
        ...
        tables = await conn.fetch(
            ...
            "WHERE table_schema='public' AND table_name IN ('users','roles','departments')"
        )
        for t in tables:
...
async def main():
    ...
    try:
        conn = await asyncpg.connect(
            host='localhost', port=5434, user='eai_flow',
            database='agentflow', timeout=5
        )
        r = await conn.fetchval('SELECT 1')
        print(f'OK  port=5434 db=agentflow user=eai_flow (no password)')
        await conn.close()
    except Exception as e:
