...
# (Cross-process writers remain a separate, pre-existing concern.) Upstream #3552.
...
async def _require_admin_user(request: Request) -> None:
    ...
    reaches this router. Falling back to the strict dependency keeps this
    route safe even in tests or alternative ASGI compositions.
    """
...
def _apply_mcp_config_update(body: McpConfigUpdateRequest) -> dict:
    ...
    Resolving the config path, the existence probe, reading the raw JSON, writing
    the merged config, and reloading it are all blocking filesystem IO that must
    stay off the event loop. The merge is pure in-memory work but lives here too
    so the whole read-modify-write is a single worker hop (upstream #3552).
...
async def update_mcp_configuration(request: Request, body: McpConfigUpdateRequest) -> McpConfigResponse:
    ...
    try:
        ...
        # Offload the blocking read-modify-write of extensions_config.json (path
        # resolve, existence probe, raw read, merged write, reload) to a worker
        # thread. The lock serializes concurrent updates within this process so
        # the RMW stays atomic now that it no longer runs inline on the event
        # loop (upstream #3552).
