...
@require_permission("runs", "create", owner_check=True, require_existing=True)
# EAI-CUSTOM (upstream-sync 2026-09-11): keep the EAI signature convention
# (plain ``str`` thread_id, no ``ThreadId`` — see _stream_existing_run note
# below) while adopting upstream's Idempotency-Key parameter (#5258).
...
async def stream_run(
    thread_id: str,  # EAI-CUSTOM: plain str per EAI signature convention (no ThreadId)
    body: RunCreateRequest,
    request: Request,
    idempotency_key: IdempotencyKeyHeader = None,
) -> StreamingResponse:
...
async def wait_run(
    ...
    # (upstream #5258; the checkpoint fetch below keeps the EAI-CUSTOM shared
    # state-accessor from the 2026-08-26 sync.)
...
# EAI-CUSTOM (upstream-sync 2026-09-05, merge a956bbc03): EAI previously
# registered GET+POST on ONE stacked-decorator function to keep OpenAPI
# operationIds unique; upstream splits them into a shared ``_stream_existing_run``
# helper plus POST (cancel-then-stream) and GET (observation-only, ``action``
# rejected with 405) wrappers. We adopt upstream's structure — the shared route
# name with per-method registration still yields unique per-method operationIds —
# and keep the EAI signature convention (plain ``str`` thread_id, no ``ThreadId``).
...
@require_permission("runs", "read", owner_check=True)
async def join_existing_run_stream(thread_id: str, run_id: str, request: Request) -> Response:
