...
class ValidationIssue:
    ...
    message: str  # 问题描述
    field_name: str | None = None  # 涉及的字段名
    source_value: Any | None = None  # 源值（用于数据一致性检查）
    target_value: Any | None = None  # 目标值
    location: str | None = None  # 位置（章节、段落等）
...
class CheckContext:
    ...
    # 用户上下文
    user_id: UUID | None = None  # 执行用户ID
    thread_id: UUID | None = None  # 会话ID
...
class BaseValidator(ABC):
    ...
    @abstractmethod
    def validate(self, context: CheckContext, rule_config: dict) -> list[ValidationIssue]:
        """
        执行验证
    ...
    def error(self, message: str, details: dict = None) -> list[ValidationIssue]:
        """返回错误结果"""
        return [ValidationIssue(rule_id="", rule_name="", severity=SeverityLevel.WARNING, check_result=CheckResult.ERROR, message=f"验证执行错误: {message}", details=details or {})]
...
class ValidatorRegistry:
    ...
    @classmethod
    def get_validator(cls, rule_type: str) -> type[BaseValidator] | None:
        """获取验证器类"""
