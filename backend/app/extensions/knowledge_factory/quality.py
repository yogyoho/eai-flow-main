...
class QualityAssessmentClient:
    ...
    def _collect_rag_sources_recursive(self, sections: list[dict], sources: set) -> None:
        for sec in sections:
    ...
    def _get_fallback_assessment(self) -> dict:
        return {
            "overall_score": 50,
            "dimensions": {
                "completeness": {"score": 50, "issues": ["评估服务暂时不可用"]},
                "accuracy": {"score": 50, "issues": ["评估服务暂时不可用"]},
                "consistency": {"score": 50, "issues": ["评估服务暂时不可用"]},
                "compliance": {"score": 50, "issues": ["评估服务暂时不可用"]},
                "freshness": {"score": 50, "issues": ["评估服务暂时不可用"]},
            },
