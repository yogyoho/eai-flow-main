...
- 【待核实】形态硬规则（机校，违者整包拒绝）：
  ① phase_analysis 必含 zone_split_rule 子对象，且 zone_split_rule **自身**（不是其嵌套
  子对象）必须有 "status" 键，值恰为 "【待核实】"；
  ② 任意嵌套层出现的 "status" 键，值只能恰是字符串 "【待核实】"——禁止自造状态值
  （如「【未采集】」「未核实」「氧化率>50%」等裸串都是违规）；信息缺失/未采集时同样写
  {{"status": "【待核实】", "note": "缺失原因"}}。
