...
async def get_timeline(db: AsyncSession, project_id: UUID) -> list[ProjectTimeline]:
    ...
    stmt = select(ProjectTimeline).where(ProjectTimeline.project_id == project_id).order_by(ProjectTimeline.planned_start)
    result = await db.execute(stmt)
    return list(result.scalars().all())


async def upsert_timeline_entry(db: AsyncSession, project_id: UUID, phase_node: str, data: dict) -> ProjectTimeline:
