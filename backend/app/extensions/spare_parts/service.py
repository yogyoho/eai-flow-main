# EAI-CUSTOM: forked from contract_price/service.py; 差异=客户维度(csp_customers + customer_id)
...
Triggers the agent-skill pipeline (``skills/public/spare-parts-analysis``) as
a subprocess so the Gateway never imports the gitignored skill package directly.
The skill's CLI writes its results to the shared ``csp_`` tables (same DB), which
this extension reads back. The run is recorded in ``csp_run_history``.
"""
...
from app.extensions.spare_parts import crud

logger = logging.getLogger(__name__)

# Skill directory resolved relative to the repo root (backend/ is cwd at runtime).
_REPO_ROOT = Path(__file__).resolve().parents[4]  # backend/app/extensions/spare_parts -> repo root
...
_SKILL_DIR = _REPO_ROOT / "skills" / "public" / "spare-parts-analysis"
