# EAI-CUSTOM: forked from contract_price/routers.py; 差异=客户维度(csp_customers + customer_id)
# + 备件域(part_name/spec);csp_run_history 无 excel_path、有 customers_resolved。
"""Spare-parts price analysis management API — all functional areas.

Mounted into the Gateway under ``/api/extensions/spare-parts``. Endpoints:
  ...
  Functional area 6 (dashboard): GET /dashboard, GET /spare-parts-analysis
  Functional area 8 (customers): GET/POST /customers, PATCH/claim/merge/resolve /customers
...
router = APIRouter(prefix="/api/extensions/spare-parts", tags=["Spare Parts Price Analysis"])
...
async def list_documents(
    ...
    customer_id: UUID | None = None,  # EAI-CUSTOM (D3): 按客户筛选文档
    skip: int = Query(0, ge=0),
    limit: int = Query(20, ge=1, le=100),
    db: AsyncSession = Depends(get_db),
    _: CurrentUser = Depends(require_permission("system:access")),  # EAI-CUSTOM: Add permission check
):
    items, total = await crud.list_documents(db, keyword, parse_status, customer_id, skip, limit)
...
async def delete_document(
    ...
    doc = await db.get(CspDocument, doc_id)
    if doc is None:
...
async def upload_document(
    ...
    """Upload a contract to the independent csp-documents MinIO bucket.
...
async def reparse_document(
    ...
    storage_uri, so the existing csp_documents row is updated in place and its
    ...
    if doc is None:
        raise HTTPException(status_code=404, detail="document not found")
    # storage_uri looks like "s3://csp-documents/<key>"
    key = doc.storage_uri.split("/", 3)[-1] if doc.storage_uri.count("/") >= 3 else doc.file_name
...
@router.get("/items/customers")
async def list_item_customers(
    db: AsyncSession = Depends(get_db),
    _: CurrentUser = Depends(require_permission("system:access")),  # EAI-CUSTOM: Add permission check
):
    """EAI-CUSTOM (D3): distinct customer_id/customer_name with item counts
    (items-page 客户筛选下拉)。"""
    return await crud.list_item_customers(db)
...
async def list_items(
    ...
    customer_id: UUID | None = None,  # EAI-CUSTOM (D3): 按客户筛选明细
...
# --- Functional area 8: customers (D3 客户主数据) --------------------------
# EAI-CUSTOM: ④ 特色 —— 客户主数据 CRUD + 认领/合并/预解析,支撑跨客户比价。
...
async def list_customers(
    ...
    items, total = await crud.list_customers(db, keyword, customer_status, skip, limit)
    return Page[CustomerOut](items=items, total=total, skip=skip, limit=limit)
...
async def update_customer(
    ...
    c = await crud.update_customer(db, customer_id, body.model_dump(exclude_unset=True))
    if c is None:
        raise HTTPException(status_code=404, detail="customer not found")
    return c


@router.post("/customers/{customer_id}/claim", response_model=CustomerOut)
async def claim_customer(
    ...
    """把一个 OCR 脏客户名认领到指定规范客户(并入 aliases,去重)。"""
    c = await crud.claim_customer(db, customer_id, body.raw_name)
    if c is None:
        raise HTTPException(status_code=404, detail="customer not found")
    return c


@router.post("/customers/merge", response_model=CustomerOut)
async def merge_customers(
    body: CustomerMerge,
    db: AsyncSession = Depends(get_db),
    _: CurrentUser = Depends(require_permission("system:access")),  # EAI-CUSTOM: Add permission check
):
    """合并 N 个客户到 target:别名并入 + 文档/明细 customer_id 回填 + source 置 merged。"""
    try:
        c = await crud.merge_customers(db, body.source_ids, body.target_id)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    if c is None:
...
@router.post("/customers/resolve")
async def resolve_customers(
    body: CustomerResolve,
    db: AsyncSession = Depends(get_db),
    _: CurrentUser = Depends(require_permission("system:access")),  # EAI-CUSTOM: Add permission check
):
    """批量预览:脏客户名 → customer_id(canonical/aliases 命中,未命中 None)。只读。"""
    return await crud.resolve_customers(db, body.raw_names)
...
@router.get("/spare-parts-analysis")
async def spare_parts_analysis(
    name: str | None = None,
    cluster_id: UUID | None = None,
    skip: int = Query(0, ge=0),
    limit: int = Query(50, ge=1, le=200),
...
async def pipeline_status(
    ...
    return {
        ...
        "customers_resolved": run.customers_resolved,  # EAI-CUSTOM (D3)
