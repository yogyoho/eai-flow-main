# EAI-CUSTOM: forked from contract_price/storage.py; 差异=客户维度(csp_customers + customer_id)
# + 备件域(part_name/spec);csp_run_history 无 excel_path、有 customers_resolved。
"""MinIO storage for contract files (independent ``csp-documents`` bucket).
...
BUCKET = "csp-documents"


def _client() -> Minio:
    """Minio client for the contract bucket. Uses CSP_MINIO_* env (default
    ...
    return Minio(
        os.environ.get("CSP_MINIO_ENDPOINT", "ragflow-minio:9000"),
        access_key=os.environ.get("CSP_MINIO_ACCESS_KEY", "minioadmin"),
        secret_key=os.environ.get("CSP_MINIO_SECRET_KEY", "minioadmin"),
        secure=os.environ.get("CSP_MINIO_SECURE", "false").lower() == "true",
    )
