...
- 其他单页模块的 tab 化拆分。
- **页面可见性是客户端 UX**：隐藏页 tab 不渲染，但深层链接直达时由**模块级** `require_permission`（如 `cpa:read`、`system:access`）兜底 403——不做页面级后端检查。与既有 nav 可见性设计一致（导航隐藏是展示层，模块权限才是强制边界）。
- `pages` 缺省语义已统一：缺 `pages` 键 / `["*"]` = 全可见；显式 `pages: []` = 全不可见（与运行时 canPage 一致）。
