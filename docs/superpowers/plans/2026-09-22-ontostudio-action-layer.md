...
| 放宽正则 / 改常量为恒真 | ✅ | |
| 清空方法体 / 让函数直接 return | ✅ | |
| **删掉装饰器** | ✅ | 对 `@model_validator` 这类**按装饰器注册**的机制，这是唯一正确的禁用方式 |
| **改函数名 / 改签名** | ❌ **假阴性** | pydantic 的 `@model_validator` 按装饰器注册、**不按函数名解析**，改名对行为零影响。实现者实测：把 `_check_refs` 改名为 `_unused_disabled` → **7 passed 全绿**，差点据此误判校验器是死代码 |

**假阴性比假阳性危险**：它会让人把一个**有保护**的实现判成**没保护**，然后去"补"一个本就存在的测试，同时把真实盲区留在阴影里。**审查者报"某段代码无测试保护"时，必须说明变异手法；若手法是改名/改签名，结论作废、须用删装饰器或清空方法体重做。**
