...
# EAI-CUSTOM: dev project name eai-docker (fork policy: all compose uses -p eai-docker).
COMPOSE_PROJECT="eai-docker"
...
_refresh_compose_cmd() {
    # EAI-CUSTOM: dev compose project name comes from COMPOSE_PROJECT (eai-docker,
    # fork policy: all compose uses -p eai-docker); upstream's parameterized form
    # is kept so `logs --prod` can swap to the production project `deer-flow`.
