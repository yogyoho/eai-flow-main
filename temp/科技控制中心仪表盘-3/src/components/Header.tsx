...
import { Terminal, Shield, Plus, BrainCircuit, Waves, Sun, Moon, Home, ArrowLeft } from "lucide-react";
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between justify-start gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <motion.button
            ...
            className={`px-3.5 py-2.5 rounded-lg border transition-all duration-300 cursor-pointer flex items-center gap-1.5 text-xs font-bold font-sans ${
              isDarkMode 
                ? "bg-slate-900/80 border-slate-800 text-cyan-400 hover:text-cyan-300 hover:bg-slate-800" 
                : "bg-slate-100 border-slate-200 text-slate-700 hover:text-slate-900 hover:bg-slate-200 shadow-sm"
            }`}
            title="返回平台门口主页 (Return to Landing)"
          >
            <Home className="w-4 h-4 text-cyan-500 animate-pulse" />
            <span className="hidden md:inline">平台门户</span>
          </motion.button>
