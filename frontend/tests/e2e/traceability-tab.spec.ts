...
test.describe("文档空间协同编辑 - 溯源 Tab", () => {
  test.beforeEach(async ({ page }) => {
    ...
    const emailInput = page
      .locator(
        'input[type="email"], input[name="email"], input[placeholder*="邮箱"], input[placeholder*="email"], input[placeholder*="Email"]',
      )
      .first();
  ...
  test("溯源 Tab - 文档编辑页中可见并可以切换", async ({ page }) => {
    ...
    await page.screenshot({
      path: "test-results/traceability-01-docmgr.png",
      fullPage: false,
    });
    ...
    const docLink = page
      .locator(
        'a[href*="docmgr"], button:has-text("打开"), [data-testid="doc-row"], .document-item, .doc-card',
      )
      .first();
    ...
    if (await docLink.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const firstClickable = page
        .locator('table tbody tr, [role="listitem"], .card, .list-item')
        .first();
      if (
        await firstClickable.isVisible({ timeout: 3000 }).catch(() => false)
      ) {
    ...
    await page.screenshot({
      path: "test-results/traceability-02-editor.png",
      fullPage: false,
    });
    ...
    if (await traceabilityBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const bookBtn = page
        .locator(
          'button:has(.lucide-book-open), button:has(svg[class*="book"])',
        )
        .first();
    ...
    await page.screenshot({
      path: "test-results/traceability-03-traceability-panel.png",
      fullPage: false,
    });
    ...
    if (panelVisible) {
      ...
      const statsGrid = page
        .locator('.grid, [class*="grid"]')
        .filter({
          has: page.locator(
            "text=rag_retrieval, text=ai_generated, text=knowledge_base, text=regulation",
          ),
        });
      ...
      if (
        await missingWarning.isVisible({ timeout: 2000 }).catch(() => false)
      ) {
      ...
      if (
        await footnoteSection.isVisible({ timeout: 2000 }).catch(() => false)
      ) {
      ...
      await page.screenshot({
        path: "test-results/traceability-03b-no-panel.png",
        fullPage: true,
      });
    }
    ...
    const editorArea = page
      .locator(
        '[contenteditable="true"], .ProseMirror, [data-content-editable]',
      )
      .first();
    ...
    await page.screenshot({
      path: "test-results/traceability-04-final.png",
      fullPage: true,
    });
  });
  ...
  test("溯源 Tab - 关闭后重新打开", async ({ page }) => {
    ...
    // Open a document
    const docLink = page
      .locator('a[href*="docmgr"], button:has-text("打开")')
      .first();
    if (await docLink.isVisible({ timeout: 5000 }).catch(() => false)) {
      await docLink.click();
      await page.waitForTimeout(3000);
    ...
    if (await traceBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      await expect(page.locator("text=溯源").first()).toBeVisible({
        timeout: 3000,
      });
      ...
      await traceBtn.click();
      await page.waitForTimeout(1000);
      const panelReopened = await page
        .locator("text=溯源")
        .first()
        .isVisible({ timeout: 3000 })
        .catch(() => false);
      console.log(
        panelReopened ? "✅ 溯源面板重新打开成功" : "⚠️ 重新打开失败",
      );
    } else {
      console.log("⚠️ 未找到溯源按钮 — 可能不在文档编辑页");
      await page.screenshot({
        path: "test-results/traceability-toggle-no-btn.png",
        fullPage: true,
      });
    }
  });
