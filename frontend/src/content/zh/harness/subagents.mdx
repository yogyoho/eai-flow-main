...
除内置子 Agent 外，EAIFlow 还通过 **Agent Connect Protocol (ACP)** 支持委派给外部 Agent。ACP 允许 EAIFlow 调用作为独立进程运行的 Agent（包括用 ACP 适配器包装的第三方 CLI 工具）。
...
<Callout type="tip">
  ACP Agent 作为 EAIFlow 管理的子进程运行，通过 ACP 协议通信。标准 CLI
