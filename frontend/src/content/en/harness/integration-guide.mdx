...
description: EAIFlow Harness is not only a standalone application. It is a Python library you can import and use inside your own backend, API server, automation system, or multi-agent orchestrator.
---
...
<Callout type="info" emoji="🔌">
  EAIFlow Harness can be embedded into any Python application. This guide
  covers the integration patterns for using EAIFlow as a library inside your
  own system.
</Callout>

EAIFlow Harness is not only a standalone application. It is a Python library you can import and use inside your own backend, API server, automation system, or multi-agent orchestrator.

## Embedding EAIFlowClient

The primary integration point is `EAIFlowClient`. It wraps the LangGraph runtime and exposes a clean API for sending messages and streaming responses from any Python application.

```python
from deerflow.client import EAIFlowClient
from deerflow.config import load_config

# Load configuration (reads config.yaml or DEER_FLOW_CONFIG_PATH)
load_config()

client = EAIFlowClient()
```
...
EAIFlow Gateway is itself a FastAPI application. You can mount it as a sub-application or router:
...
# Mount the EAIFlow gateway router
from deerflow.app.gateway.main import app as gateway_app
app.mount("/deerflow", gateway_app)
```

Or use `EAIFlowClient` directly in your own FastAPI routes with streaming:
...
EAIFlow Harness is built on LangGraph. The Lead Agent is a standard LangGraph graph. You can compose it with your own LangGraph nodes and graphs:
...
EAIFlow can expose its agent as an MCP server, allowing other MCP-compatible systems to call it as a tool. Refer to the EAIFlow repository for MCP server integration examples.
