...
import { useDeleteRun, useRuns } from "@/extensions/spare-parts/hooks";
...
export function TasksView() {
  ...
  return (
    <div className="space-y-6 p-8">
      <PageHeader
        title="任务中心"
        description="手动与定时分析任务的运行记录（含客户归并统计）。"
          ...
          <Button
            ...
            onClick={() => {
              if (!confirm(`确认删除选中的 ${selected.size} 条运行记录？`))
                return;
              void Promise.all(
                [...selected].map((id) => deleteRun.mutateAsync(id)),
              )
                .catch((e) => console.error("批量删除运行记录失败", e))
