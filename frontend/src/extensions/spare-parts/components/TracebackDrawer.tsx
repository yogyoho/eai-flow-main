...
import { Crosshair, Loader2, Maximize2, X, ZoomIn, ZoomOut } from "lucide-react";
...
export function TracebackDrawer({ docId, page, bbox, onClose }: Props) {
  ...
  const onMouseMove = (e: React.MouseEvent) => {
    ...
    if (!d) return;
    setView((v) => ({ ...v, x: d.vx + (e.clientX - d.mx), y: d.vy + (e.clientY - d.my) }));
  };
  const endDrag = () => {
      ...
      <div className="relative w-full max-w-2xl h-full bg-card border-l border-border shadow-2xl overflow-auto">
        ...
        <div className="p-5 space-y-3">
          {loading ? (
            ...
            <div
              ref={viewportRef}
              className="relative w-full overflow-hidden rounded-md border border-border bg-muted/20 select-none touch-none"
            ...
            >
              ...
              {/* zoom toolbar */}
              <div className="absolute top-3 right-3 z-10 flex flex-col items-center gap-0.5 rounded-lg border border-border bg-background/90 p-1 shadow-md backdrop-blur">
                <button onClick={() => zoomFromCenter(1.4)} disabled={view.scale >= MAX_SCALE} className={iconBtn} aria-label="放大">
                  <ZoomIn className="w-4 h-4" />
                </button>
                <span className="text-[10px] font-cyber db-text-subtle leading-tight">
                  {Math.round(view.scale * 100)}%
                </span>
                <button onClick={() => zoomFromCenter(1 / 1.4)} disabled={view.scale <= MIN_SCALE} className={iconBtn} aria-label="缩小">
                  <ZoomOut className="w-4 h-4" />
                </button>
                <button onClick={reset} disabled={atRest} className={iconBtn} aria-label="重置缩放">
          ...
          <p className="text-xs db-text-subtle font-cyber tracking-wide">
            红框 = OCR 提取位置 · 对照原文核验价格数字是否正确 · 滚轮/点击缩放，放大后可拖动平移
          </p>
        </div>
      </div>
    </div>
