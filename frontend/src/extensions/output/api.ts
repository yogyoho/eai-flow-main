...
export const outputApi = {
  ...
  importLayout: async (file: File): Promise<Record<string, unknown>> => {
    ...
    return authFormFetch<Record<string, unknown>>(
      `${API_BASE}/import-layout`,
      form,
    );
  },

  createTemplate: async (
    tpl: Omit<LayoutTemplate, "id" | "isBuiltin" | "createdAt" | "updatedAt">,
    ...
    const resp = await authFetch<Record<string, unknown>>(
      `${API_BASE}/templates`,
      {
        method: "POST",
  ...
  updateTemplate: async (
    id: string,
    tpl: Partial<
      Omit<LayoutTemplate, "id" | "isBuiltin" | "createdAt" | "updatedAt">
    >,
  ): Promise<LayoutTemplate> => {
    const payload: Record<string, unknown> = {};
    if (tpl.name !== undefined) payload.name = tpl.name;
    if (tpl.reportType !== undefined) payload.report_type = tpl.reportType;
    if (tpl.pageSettings !== undefined)
      payload.page_settings = tpl.pageSettings;
    if (tpl.bodyStyles !== undefined) payload.body_styles = tpl.bodyStyles;
    if (tpl.headingStyles !== undefined)
      payload.heading_styles = tpl.headingStyles;
    if (tpl.coverTemplate !== undefined)
      payload.cover_template = tpl.coverTemplate;
    if (tpl.coverMaster !== undefined) payload.cover_master = tpl.coverMaster;
    if (tpl.coverElements !== undefined)
      payload.cover_elements = tpl.coverElements;
    ...
    if (tpl.figureStyles !== undefined)
      payload.figure_styles = tpl.figureStyles;
    if (tpl.headerFooter !== undefined)
      payload.header_footer = tpl.headerFooter;
    if (tpl.referenceStyle !== undefined)
      payload.reference_style = tpl.referenceStyle;
    if (tpl.appendixRules !== undefined)
      payload.appendix_rules = tpl.appendixRules;
  ...
  generate: async (
    ...
    if (req.source === "markdown" && req.markdownFile) {
      ...
      const data = await authFormFetch<Record<string, unknown>>(
        `${API_BASE}/generate`,
        form,
      );
      return {
