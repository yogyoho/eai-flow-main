...
export default function ExtractionTaskModal({ onClose, onSuccess }: Props) {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        if (!res.ok) {
          ...
          if (res.status === 403) {
            throw new Error(
              `无权限上传文件（${detail ?? "请确认已登录并有知识库上传权限"}）`,
            );
          }
  ...
  useEffect(() => {
    ...
    kfApi
      ...
      .catch(() => {
        // intentional no-op: template list load failure leaves previous state
      });
    return () => {
      cancelled = true;
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const data: ExtractionTaskCreate = {
        ...
        uploaded_file_ids:
          uploadedDocIds.length > 0 ? uploadedDocIds : undefined,
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="text-muted-foreground file:bg-primary/10 file:text-primary hover:file:bg-primary/20 text-sm file:mr-3 file:rounded-md file:border-0 file:px-3 file:py-1.5 file:text-sm disabled:opacity-50"
                />
                {uploadingFiles && (
