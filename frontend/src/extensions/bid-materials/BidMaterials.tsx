...
// EAI-CUSTOM (Plan 4): 投标资料管理主组件——资质版本库 + 标书样例台账 + 大纲候选回看三 tab。
// 镜像 eia-samples/SampleLibrary.tsx 的 tab 形态(Tabs+TabsContent, Shadcn)。
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";

import { OutlineCandidates } from "./OutlineCandidates";
    ...
    <div className="mx-auto flex h-full w-full max-w-7xl flex-col gap-4 p-4">
      <div>
        ...
        <p className="text-muted-foreground text-sm">
          投标资质版本库（MinIO 代理、到期预警）、标书技术资料台账（bank_compile
          样例）与大纲候选回看（线程候选 JSON 只读）。
        </p>
      </div>
      <Tabs
      ...
      >
        <TabsList>
          ...
          <TabsTrigger value="outline">大纲候选</TabsTrigger>
        </TabsList>
