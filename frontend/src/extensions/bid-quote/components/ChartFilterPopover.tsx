...
// EAI-CUSTOM: 基于 ui/popover(Radix)而非手写面板 —— 外点关闭/Escape/焦点管理由 primitive 提供。
export function ChartFilterPopover({ chart, onChange, enable }: Props) {
  ...
  // !! 收窄为布尔: selfAttribute 是可 truthy 字符串("all"),?? 会改变语义
      ...
      <PopoverContent align="end" className="w-52 p-2">
        {enable.selfAttribute && (
          <div className="mb-2" role="group" aria-label="自产属性">
            ...
            {(
              ["all", "self_dominant", "outsource_dominant"] as SelfAttribute[]
            ).map((a) => (
              <button
                key={a}
