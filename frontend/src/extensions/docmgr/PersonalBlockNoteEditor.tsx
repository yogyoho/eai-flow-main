...
/** Structural subset of a BlockNote block used for anchor matching / snapshots. */
...
interface TableCellStyle {
  content?: TextNodeLike[];
}
interface TableRowStyle {
  cells?: TableCellStyle[];
}
interface TableContentStyle {
  type?: string;
  rows?: TableRowStyle[];
}

function getBlockText(block: AnchorBlock | null | undefined): string {
  if (!block) return "";
  // Table: collect text from all cells
  if (block.type === "table") {
    const content = block.content as TableContentStyle | undefined;
  ...
  if (Array.isArray(block.content)) {
    return (block.content as TextNodeLike[])
...
export function findBlockByAnchor(
  ...
  for (let i = 0; i < doc.length; i++) {
    const block = doc[i];
    if (!block) continue;
    const fullText = normalizeAnchorText(getBlockText(block));
    if (!fullText) continue;
    if (fullText === trimmed) return { blockId: block.id, blockIndex: i };
  }
  ...
  for (let i = 0; i < doc.length; i++) {
    ...
    const fullText = normalizeAnchorText(getBlockText(block));
    if (!fullText) continue;
    if (fullText.startsWith(trimmed))
      return { blockId: block.id, blockIndex: i };
  }
  ...
  for (let i = 0; i < doc.length; i++) {
    ...
    if (fullText.includes(trimmed)) {
      ...
      containsMatch = { blockId: block.id, blockIndex: i };
    }
  }
  ...
  for (let i = 0; i < doc.length; i++) {
    const block = doc[i];
    if (!block) continue;
    let fullText = normalizeAnchorText(getBlockText(block));
    if (!fullText) continue;
    ...
    if (fullDist / trimmed.length < 0.3 && (!best || fullDist < best.dist)) {
      best = { blockId: block.id, blockIndex: i, dist: fullDist };
    }

    if (fullText.length > trimmed.length + 5) {
      const windowLen = Math.max(
        trimmed.length,
        Math.min(trimmed.length * 2, fullText.length),
      );
      for (let start = 0; start <= fullText.length - trimmed.length; start++) {
        ...
        if (dist / trimmed.length < 0.3 && (!best || dist < best.dist)) {
          best = { blockId: block.id, blockIndex: i, dist };
          if (dist === 0) break;
        }
      }
    }
...
export interface PersonalBlockNoteEditorRef {
  ...
  restoreBlocks: (blocks: unknown[]) => void;
...
>(({ initialContent, onChange, className, hideSideMenu, threadId }, ref) => {
  ...
  const dictionary = useMemo(
    () =>
      (isZh ? { ...coreZh, ai: aiZh } : { ...coreEn, ai: aiEn }) as Dictionary,
    [isZh],
  );
  ...
  }, [editor, initialContent, seeded, rebuildHeadings]);
        ...
        void (async () => {
          ...
          try {
            ...
            for (const block of doc) {
              ...
              if (block.type === "table") {
                const tc = block.content as TableContentStyle | undefined;
                if (tc?.type === "tableContent" && Array.isArray(tc.rows)) {
                  let tableChanged = false;
                  const newRows = tc.rows.map((row: TableRowStyle) => {
                    if (!Array.isArray(row.cells)) return row;
                    const newCells = row.cells.map((cell: TableCellStyle) => {
                      if (!cell || !Array.isArray(cell.content)) return cell;
                      let changed = false;
                      const newContent: unknown[] = cell.content
                        .map((node: TextNodeLike): unknown => {
              ...
              const { content: newContent, changed } =
                convertInlineMathInContent(block.content);
              if (changed) {
                editor.updateBlock(block, {
                  type: block.type,
                  content: newContent,
          ...
          let md = "";
          try {
            const blocksForExport = prepareBlocksForMarkdownExport(
            ...
            console.warn(
              "[PersonalBN] blocksToMarkdownLossy failed:",
  ...
  useImperativeHandle(ref, () => ({
      ...
      editor.blocksToMarkdownLossy(
        prepareBlocksForMarkdownExport(
          editor.document as unknown as Parameters<
            typeof prepareBlocksForMarkdownExport
          >[0],
        ) as unknown as Parameters<typeof editor.blocksToMarkdownLossy>[0],
      ),
    ...
    getBlockAnchors: () => {
      return editor.document.map((b, i) => {
        ...
        if (b.type === "heading") {
          const level = (b.props as Record<string, unknown> | undefined)?.level;
          if (typeof level === "number") {
            anchor.headingLevel = level;
          }
        }
        return anchor;
      });
    ...
    applyOperations: (ops: DocOperation[]) => {
      for (const op of ops) {
        ...
        if (parsed.length === 0 && op.op !== "delete") {
          console.warn(
            "[applyOperations] skip: empty parsed content for op",
            op.op,
          );
          continue;
        }
    ...
    findText: (query: string) => {
      ...
      editor.document.forEach((b, blockIndex) => {
        const text = getBlockText(b);
        if (!text) return;
    ...
    replaceInBlock: (blockId: string, query: string, replacement: string) => {
      ...
      const { content: newContent, replaced } = replaceTextInContent(
        block.content,
        query,
        replacement,
      );
      if (replaced > 0)
        editor.updateBlock(block, { content: newContent } as Parameters<
        ...
        return [
          {
            key: "accept",
            title: "替换",
            onItemClick: () => ai.acceptChanges(),
          },
          {
            key: "reject",
            title: "撤销",
            onItemClick: () => ai.rejectChanges(),
          },
        ];
      }
        ...
        ? [
            {
              key: "ai-polish",
              title: "润色",
              onItemClick: (setPrompt: (p: string) => void) =>
                setPrompt(
                  "润色选中文本，使其更加流畅、专业，保持原意不变。只输出润色后的文本。",
                ),
            },
            {
              key: "ai-expand",
              ...
              onItemClick: (setPrompt: (p: string) => void) =>
                setPrompt(
                  "扩写选中文本，增加更多细节、论据或说明，使内容更加丰富详实。只输出扩写后的文本。",
                ),
            },
            {
              key: "ai-condense",
              ...
              onItemClick: (setPrompt: (p: string) => void) =>
                setPrompt(
                  "精简选中文本，去除冗余内容，保留核心信息，使表达更加简洁有力。只输出精简后的文本。",
                ),
            },
            {
              key: "ai-continue",
              ...
              onItemClick: (setPrompt: (p: string) => void) =>
                setPrompt(
                  "基于选中文本内容继续撰写，保持风格和主题一致。只输出续写的部分。",
                ),
            },
          ]
        : [
            ...
            },
            {
              ...
              onItemClick: (setPrompt: (p: string) => void) =>
                setPrompt(
                  "基于当前文档内容进行头脑风暴，提供3-5个相关的扩展思路或角度。只输出思路列表。",
                ),
            },
            {
              key: "ai-outline",
              ...
              onItemClick: (setPrompt: (p: string) => void) =>
                setPrompt(
                  "根据当前文档的上下文，生成一个合适的大纲结构。使用 Markdown 标题格式。",
                ),
            },
          ];
    },
