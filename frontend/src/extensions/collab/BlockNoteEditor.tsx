...
import {
  registerHumanWrittenPlugin,
  resetHumanWrittenTracking,
} from "./human-written-plugin";
...
/** Raw source row returned by the traceability sources endpoint (camelCased). */
interface TraceabilitySourceRow {
    ...
    if (this.state.hasError) {
      return (
        <div className="text-muted-foreground p-4 text-sm">
          <p className="mb-2 font-medium">编辑器加载失败</p>
          <pre className="bg-muted rounded p-2 text-xs break-all whitespace-pre-wrap">
...
function CommentToolbarButton({
  ...
  return (
    ...
    >
      <Components.FormattingToolbar.Button mainTooltip={"评论"}>
...
>(function BlockNoteEditor(
  { documentId, initialContent, projectId, chapterId, visibleChapterIds },
  ref,
) {
  const { ydoc, connected, synced, users, broadcastEvent } =
  ...
  const {
    ...
    reopenComment,
    deleteComment,
  } = useComments(documentId, broadcastEvent);
  const {
    versions,
  ...
  const [inlineThreadBlockId, setInlineThreadBlockId] = useState<string | null>(
    null,
  );
  ...
  const popoverVirtualRef = useRef<{
  ...
  const editor = useCreateBlockNote(
    {
      ...
      dictionary: { ...coreEn, ai: aiEn } as Dictionary &
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error(
        "[BlockNoteEditor] Failed to seed collaborative content:",
        error,
      );
    }
    ...
    workflowApi
      ...
      .then((data) => {
        ...
        const sources: TraceabilitySource[] = (data.sources ?? []).map(
          (s: TraceabilitySourceRow, i: number) => ({
      ...
      .catch((err) => {
        console.warn(
          "[BlockNoteEditor] Failed to fetch traceability sources:",
          err,
        );
      });
  ...
  return (
    <div className="flex h-full min-h-0 flex-1">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="border-border flex items-center justify-between border-b px-4 py-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <Button
              ...
              onClick={() =>
                setSidePanel(sidePanel === "versions" ? null : "versions")
              }
              title="版本历史"
            >
            ...
            {projectId && (
              <Button
                size="icon"
                variant={sidePanel === "traceability" ? "secondary" : "ghost"}
                onClick={() =>
                  setSidePanel(
                    sidePanel === "traceability" ? null : "traceability",
                  )
                }
                title="溯源"
              >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          ...
          >
            ...
            {Array.from(commentsByBlock.entries()).map(
              ([blockId, blockComments]) => (
                <BlockCommentAnchor
                  key={blockId}
                  blockId={blockId}
            ...
            {inlineThreadBlockId &&
              commentsByBlock.has(inlineThreadBlockId) && (
                <InlineCommentThread
          ...
          >
            <PopoverAnchor
              virtualRef={
                popoverVirtualRef as React.RefObject<{
                  getBoundingClientRect: () => DOMRect;
                }>
              }
            />
      ...
      {sidePanel === "ai" && (
        <div className="border-border bg-background flex h-full w-80 flex-col border-l">
          <div className="border-border flex items-center justify-between border-b p-3">
            <span className="text-sm font-medium">AI 文档审查</span>
            <Button
              size="icon"
              variant="ghost"
            ...
            <AIDocumentReview
              ...
              onInsertComment={(blockId, comment) => {
                handleCreateComment(
                  ...
                  console.error(
                    "[BlockNoteEditor] Failed to insert AI review comment:",
                    e,
                  );
                });
                setSidePanel("comments");
      ...
      {sidePanel === "traceability" && projectId && (
        <div className="border-border bg-background flex h-full w-80 flex-col border-l">
          <div className="border-border flex items-center justify-between border-b p-3">
            <span className="text-sm font-medium">溯源</span>
            <Button
              size="icon"
              variant="ghost"
              onClick={() => setSidePanel(null)}
