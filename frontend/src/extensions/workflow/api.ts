...
export const workflowApi = {
  ...
  create: async (req: CreateWorkflowRequest): Promise<WorkflowDefinition> => {
    const data = await authFetch<Record<string, unknown>>(
      `${API_BASE}/definitions`,
      {
        method: "POST",
        body: JSON.stringify(
          toSnakeCase(req as unknown as Record<string, unknown>),
        ),
      },
    );
    return toCamelCase<WorkflowDefinition>(data);
  ...
  delete: async (id: string): Promise<void> => {
    await authFetch<void>(`${API_BASE}/definitions/${id}`, {
      method: "DELETE",
    });
  },
  ...
  publishTemplate: async (id: string): Promise<WorkflowDefinition> => {
    const data = await authFetch<Record<string, unknown>>(
      `${API_BASE}/definitions/${id}/publish-template`,
      {
        method: "POST",
      },
    );
  ...
  getSources: async (
    projectId: string,
    chapterId: string,
  ): Promise<{ sources: ChapterSource[]; stats: Record<string, number> }> => {
    const data = await authFetch<Record<string, unknown>>(
      `${API_BASE}/projects/${projectId}/chapters/${chapterId}/sources`,
    );
    return toCamelCase<{
  ...
  getMissingSources: async (
    projectId: string,
    chapterId: string,
  ): Promise<{ missing: Array<{ blockIndex: number; preview: string }> }> => {
    const data = await authFetch<Record<string, unknown>>(
      `${API_BASE}/projects/${projectId}/chapters/${chapterId}/sources/missing`,
    );
    return toCamelCase<{
      missing: Array<{ blockIndex: number; preview: string }>;
    }>(data);
  },
  ...
  assignReviews: async (
    ...
    const body = {
      ...
      assignments: assignments.map((a) =>
        toSnakeCase(a as unknown as Record<string, unknown>),
      ),
    };
    const data = await authFetch<Record<string, unknown>[]>(
  ...
  startWorkflow: async (
    projectId: string,
    workflowId: string,
  ): Promise<{ status: string; temporalWorkflowId: string }> => {
    const data = await authFetch<Record<string, unknown>>(
  ...
  submitApproval: async (id: string): Promise<TemplateApproval> => {
    const data = await authFetch<Record<string, unknown>>(
      `${API_BASE}/definitions/${id}/submit-approval`,
      {
        method: "POST",
      },
    );
  ...
  reviewApproval: async (
    ...
    const data = await authFetch<Record<string, unknown>>(
      `${API_BASE}/definitions/${id}/review-approval`,
      {
        method: "POST",
  ...
  getApprovals: async (id: string): Promise<TemplateApproval[]> => {
    const data = await authFetch<Record<string, unknown>[]>(
      `${API_BASE}/definitions/${id}/approvals`,
    );
  ...
  withdrawApproval: async (id: string): Promise<{ status: string }> => {
    return authFetch<{ status: string }>(
      `${API_BASE}/definitions/${id}/withdraw-approval`,
      {
        method: "POST",
      },
    );
