...
export function SubflowConfigPanel({
  data,
  nodeId,
  onUpdate,
  orgDeptCode,
  onOrgBindingChange,
}: SubflowConfigPanelProps) {
  const [newRoleKey, setNewRoleKey] = useState("lead");
  const [departments, setDepartments] = useState<DeptItem[]>([]);

  const roles = data.requiredRoles ?? [];
  const orgDeptOptions = departments.map((d) => ({
    value: d.code ?? d.id,
    label: d.name,
  }));
  const availableRoleOptions = PRESET_ROLES.filter(
    (pr) => !roles.some((r) => r.roleKey === pr.roleKey),
  ...
  function addRole() {
    ...
    onUpdate({
      requiredRoles: [
        ...roles,
        { roleKey: preset.roleKey, count: 1, label: preset.label },
      ],
    });
  }
  ...
  function updateRoleCount(roleKey: string, count: number) {
    onUpdate({
      requiredRoles: roles.map((r) =>
        r.roleKey === roleKey ? { ...r, count: Math.max(1, count) } : r,
      ),
    });
  }
    ...
    <div className="space-y-4 p-4">
      ...
      <div className="space-y-1.5">
        ...
        <input
          ...
          className="border-border bg-background w-full rounded-md border px-2.5 py-1.5 text-sm transition-colors focus:border-violet-400 focus:ring-2 focus:ring-violet-500/20 focus:outline-none"
        />
      </div>
      ...
      <StyledCheckbox
        ...
        onChange={(v) => onUpdate({ aiAssist: v })}
        label="AI 辅助"
      />
            ...
            {roles.map((slot) => (
              ...
              >
                ...
                <input
                  ...
                  className="border-border bg-background w-11 rounded-md border px-1 py-1 text-center text-xs transition-colors focus:border-violet-400 focus:ring-2 focus:ring-violet-500/20 focus:outline-none"
                  title="人数"
                />
                <button
                  type="button"
      ...
      <NotificationsConfigPanel
        ...
        onUpdate={(notifications: NotificationConfig[]) =>
