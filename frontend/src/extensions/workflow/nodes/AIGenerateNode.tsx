...
export function AIGenerateNode({ data, selected }: NodeProps & { data: DAGNodeData }) {
  return (
    ...
    >
      ...
      <div className="text-xs font-semibold text-blue-700 dark:text-blue-300">{data.label || "AI 生成"}</div>
      {data.chapterRange && data.chapterRange.length > 0 && (
        <div className="text-[10px] text-muted-foreground mt-1">章节: {data.chapterRange.join("-")}</div>
      )}
      {data.aiAssist && <div className="text-[10px] text-blue-600 dark:text-blue-400 mt-1">AI 辅助</div>}
